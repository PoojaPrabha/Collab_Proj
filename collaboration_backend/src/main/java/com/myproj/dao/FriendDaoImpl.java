...
public class FriendDaoImpl implements FriendDao {
	...
	public List<User> listOfSuggestedUsers(String email) {
		...
		SQLQuery sqlQuery = session.createSQLQuery( "select * from user_myproj where email in (select email from user_myproj where email!=? minus (select fromId from friend_myproj where toId=? union select toId from friend_myproj where fromId=?))");
	...
	public List<User> listOfFriends(String email) {
		Session session = sessionFactory.getCurrentSession();
		SQLQuery query = session.createSQLQuery("select * from user_myproj where email in"
				+ "(select fromId from friend_myproj where toId=? and status='A' " + " union "
				+ "select toId from friend_myproj where fromId=? and status='A')");
		query.setString(0, email);
