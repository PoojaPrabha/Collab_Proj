...
app.controller('FriendController', function($scope, $location, FriendService,
	...
	// friendRequest:
	// {'id':754,'fromId':'john.s@xyz.com','toId':'james.s@xyz.com',status:'P'}
	// onclick Accept , status='A'
	// onclick Delete, status='D'
	// after assignment stat
	// {'id':754,'fromId':'john.s@xyz.com','toId':'james.s@xyz.com',status:'A'}[or
	// 'D']
	$scope.updateFriendRequest = function(friendRequest, status) {
		friendRequest.status = status // status is updated in the JSON object
		// [A/D]
		// friendRequest object status is updated to either A or D
		FriendService.updateFriendRequest(friendRequest).then(
				function(response) {
					getPendingRequests()
				}, function(response) {
					$rootScope.error = response.data
